...
let update = function() {
  ...
  if (
    ...
    oppNum++;
    oppImage.src =
      oppImageStock[Math.floor(Math.random() * oppImageStock.length)];
...
let render = function() {
  ...
  if (oppReady) {
    context.drawImage(oppImage, oppX, oppY);
  }
  if (catcherReady) {
    context.drawImage(catcherImage, catcherX, catcherY);
  }
  if (treeReady) {
...
function obstacle() {
  if (
    ...
    directionX = -directionX;
    directionY = -directionY;
    catcherX += directionX * 5;
    catcherY -= directionY * 5;
  }
}
